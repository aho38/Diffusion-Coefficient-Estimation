...
df = pd.read_csv(parent_dir / 'lake_data' /'synthetic_data_double_data.csv')
x_array, utrue1_array, utrue2_array = np.array(df['x']), np.array(df['utrue1']), np.array(df['utrue2'])
...
def loop_run(gamma, nx, a, b, omega, oce_val, beta1, beta2, noise_level, save_path,opt_csv_name,seed=0):
    ...
    ud1, ud2, A1, _, A2, _ = run.fwd_solve(m_true)
    ...
    u1_noise = u1_noise_interp(x)
    u2_noise = u2_noise_interp(x)
    ud1 = ud1.compute_vertex_values() + u1_noise * noise_level
    ud2 = ud2.compute_vertex_values() + u2_noise * noise_level
    # ud1 = true_solution1(x) + u1_noise * noise_level
    # ud2 = true_solution2(x) + u2_noise * noise_level
    # run.data_setup(ud1.compute_vertex_values(), ud2.compute_vertex_values(), normalize=False, noise_level=noise_level)
    run.data_setup(ud1, ud2, normalize=False, noise_level=noise_level)
    ...
    # ax[0].plot(x,np.exp(mtrue_array))
...
def main(seed,noise_level):   
    ...
    if beta1 == 0.5 and beta2 == 0.5:
        ...
        # dir_name = f'double_data_nx_{int(nx)}_noise_{noise_level_str}_preconditioned3/gamma_loop_double_data_nx_{nx}_noise_{noise_level_str}_seed_{seed_str}'
        dir_name = f'TESTING_DETELETE_LATER2/gamma_loop_double_data_nx_{nx}_noise_{noise_level_str}_seed_{seed_str}'
