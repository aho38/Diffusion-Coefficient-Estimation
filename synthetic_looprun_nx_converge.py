...
def loop_run(gamma, nx, a,b, bc_type, omega, oce_val, noise_level, save_path, opt_csv_name,solar_forcing_mag, u0L = 0.5, u0R = 1, seed=0):
    ...
    # add true to save_dict
    run.save_dict['u_true'] = u_true.tolist()
    run.save_dict['noise_level'] = noise_level
...
def main():
    ...
    solar_forcing_mag_list = np.linspace(0,10,121)
    # solar_forcing_mag = 6.0
    ...
    dir_name = 'Paper_story_exploartion_solar_mag_change_const_rhs_larger_REG_m_bc_fixed_WITH_NOISE'
    # dir_name = f'gamma_change_noise_{int(noise_level*100)}_pct_seed_{seed}'
    ...
    #     print(f'iter: {i}, gamma: {gamma:1.2e}')
        try:
            ## optimize loop
            idx_str = f'0{i}' if i < 10 else f'{i}' 
            opt_csv_name_str = f'opt_log{idx_str}.csv'
            _, _, _, _, _, _, _, save_dict = loop_run(gamma, nx, a,b, bc_type, omega, oce_val,noise_level, save_path, opt_csv_name_str, solar_forcing_mag, u0L, u0R, seed=seed)
            ...
            if 'nx_list' in locals():
                failed_iter[i] = nx
            elif 'solar_forcing_mag_list' in locals():
                failed_iter[i] = solar_forcing_mag
            elif 'omega_list' in locals():
                failed_iter[i] = omega
            elif 'gamma_list' in locals():
                failed_iter[i] = gamma
            else:
                failed_iter[i] = 'unknown'
            pass
    print('failed_iter: ',failed_iter)
